...
echo "This are the list of files greater than size 1GB and server whose public IP Adrress and private IP Address is given below:" >> output.txt 

find / -type f -size +1G -exec du -h 2>/dev/null {} + | sort -rh  >> output.txt
#Serach for server IPAddress
ifconfig |grep Bcast  >> output.txt 
#Send Emaiil only if file is greater than 1GB
if
